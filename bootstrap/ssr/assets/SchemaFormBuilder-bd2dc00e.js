...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    async function fetchEditFormData() {
      if (["edit"].includes(modalType.value) && props.resource) {
    ...
    async function fetchResourceData() {
      if (["modal"].includes(modalType.value) && props.resource) {
      ...
      if (modalType.value == "create") {
        formCreate.post(route(`${props.resourceRoute}.store`), {
          onSuccess: (params) => {
            console.log(params);
            closeCreateForm();
            emit("showNotify", modalType.value);
          }
        });
      }
      if (["import"].includes(modalType.value)) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$4, mergeProps({
        "modal-type": modalType.value.value,
        show: __props.modalIsOpen,
        "max-width": "2xl",
        onEdit: ($event) => modalType.value = "edit",
