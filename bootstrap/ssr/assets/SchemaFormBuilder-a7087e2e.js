    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.columns, (column) => {
        ...
        if (!["image", "id"].includes(column)) {
          _push(`<!--[--><span class="mr-2 uppercase secondary-text">${ssrInterpolate(_ctx.__(column))}</span><div class="${ssrRenderClass({
            "rounded w-14 flex justify-center": column === "is_enabled"
          })}" class="${ssrRenderClass({
            "bg-green-400 font-semibold ": __props.resourceModal[column] === 1,
            "bg-red-400 font-semibold": __props.resourceModal[column] === 0
          })}">`);
          if (["description"].includes(column)) {
            ...
            if (!readActivated.value) {
              ...
              if (!readActivated.value) {
                _push(`<span class="font-extrabold read">..read more</span>`);
              } else {
                _push(`<!---->`);
              }
              _push(`</p>`);
            ...
            _push(`<div class="flex"><p class="text-sm font-bold">${ssrInterpolate(_ctx.__(
              applyFormat(
                column,
                __props.resourceModal[column] ?? __props.resource[column]
              )
            ))}</p></div>`);
          }
          _push(`</div><!--]-->`);
