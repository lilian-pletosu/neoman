              ...
              createVNode("div", {
                ...
                unref(cartStore).products.length !== 0 ? (openBlock(), createBlock("div", {
                  ...
                  createVNode("div", { class: "flex justify-between text-base font-medium text-gray-900" }, [
                    ...
                    createVNode("p", null, toDisplayString(unref(cartStore).totalPrice.toFixed(2)) + " " + toDisplayString(_ctx.__("lei")), 1)
                  ]),
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (__props.visible) {
        ...
        if (__props.type === "buy_1_click") {
          _push(`<!--[--><span class="flex text-center mb-4 font-mulish font-medium">${ssrInterpolate(_ctx.__("for_fast_order_complete_phone_number"))}</span>`);
        ...
        if (__props.type === "buy_in_credit") {
          ...
          if (success.value) {
            _push(`<div><div class="bg-white p-6 md:mx-auto"><svg viewBox="0 0 24 24" class="text-green-600 w-16 h-16 mx-auto my-6"><path fill="currentColor" d="M12,0A12,12,0,1,0,24,12,12.014,12.014,0,0,0,12,0Zm6.927,8.2-6.845,9.289a1.011,1.011,0,0,1-1.43.188L5.764,13.769a1,1,0,1,1,1.25-1.562l4.076,3.261,6.227-8.451A1,1,0,1,1,18.927,8.2Z"></path></svg><div class="text-center"><h3 class="md:text-2xl text-base text-gray-900 font-semibold text-center">${ssrInterpolate(_ctx.__("order_placed"))}!</h3><p class="text-gray-600 my-2">${ssrInterpolate(_ctx.__("thanks_order"))}</p><p>${ssrInterpolate(_ctx.__("order_success_message"))}</p><div class="py-10 text-center"><p class="px-12 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 cursor-pointer rounded-md">${ssrInterpolate(_ctx.__("back_to_shop"))}</p></div></div></div></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<!--]-->`);
