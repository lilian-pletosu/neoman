    ...
    <div v-for="column in columns" class="mb-2">
        <template v-if="!['image', 'id'].includes(column)">
            ...
            <div
                ...
                :class="{
                    'bg-green-400 font-semibold ': resourceModal[column] === 1,
                    'bg-red-400 font-semibold': resourceModal[column] === 0,
                }"
            >
                <template v-if="['description'].includes(column)">
                    <div>
                        <p class="" v-if="!readActivated">
                            {{
                                resourceModal[column]
                                    ? resourceModal[column].slice(0, 50)
                                    : "-"
                            }}
                            <span
                                class="font-extrabold read"
                                v-if="!readActivated"
                                @click="readActivated = !readActivated"
                                >..read more</span
                            >
                        </p>
                        <p class="font-bold" v-if="readActivated">
                            {{ applyFormat(column, resourceModal[column]) }}
                        </p>
                    </div>
                </template>
