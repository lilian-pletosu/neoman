...
<template>
    <FrontLayout :title="subSubcategory.name"
            ...
            <div>
                ...
                <ReusableSidebar :open="mobileFiltersOpen" :title="__('filter')" class="lg:hidden"
                    ...
                    <template v-slot:content>
                        <div class="px-4">
                            <form class="block">
                                <Disclosure v-slot="{ open }" as="div" class="py-6 border-b border-gray-200">
                                    ...
                                    <DisclosurePanel :unmount="true" class="pt-6">
                                        <div class="">
                                            <div class="flex justify-around space-x-2">
                                                <input v-model="priceRange[0]" :min="0" class="w-full h-8 rounded-sm"
                                                    :placeholder="__('min')" type="number" />
                                                <input v-model="priceRange[1]" :min="0" class="w-full h-8 rounded-sm"
                                                    :placeholder="__('max')" type="number" />
                                            </div>
                                        </div>
                                    </DisclosurePanel>
                                </Disclosure>
                                <!--               Brand                 -->
                                <Disclosure v-for="brand in brands" v-slot="{ open }" as="div"
...
option, optionIdx
                                ...
                                <!--               Attributes                 -->
                                <Disclosure v-for="attribute in computedAttributes" v-slot="{ open }" as="div"
                                    class="py-6 border-b border-gray-200">
                                    <h3 class="flow-root -my-3">
                                        <DisclosureButton
                                            class="flex items-center justify-between w-full py-3 text-sm text-gray-400 bg-white hover:text-gray-500">
                                    ...
                                    <DisclosurePanel v-if="attribute.name" class="pt-6">
                                        <div class="space-y-4">
                                            <div v-for="option in attribute.options" class="flex items-center">
                                                <input :checked="isOptionSelected(
                                                    attribute.key,
                                                    option.id
                                                )
                                                    " :name="option.value" :value="option.id"
                                                    class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                ...
                <main class="max-w-full px-4 mx-auto bg-white sm:px-6 lg:px-8 dark:bg-dark">
                    <div
                        class="flex items-center justify-between pt-4 pb-2 border-b border-gray-200 text-dark dark:text-white">
                        <h1 class="text-lg font-bold font-mulish md:text-xl lg:text-2xl">
                            {{ subSubcategory.name }}
                        </h1>
                        <div class="flex items-center">
                                ...
                                <div>
                                    <MenuButton
                                        ...
                                        <span class="flex sm:hidden">
                                            <PresentationChartBarIcon aria-hidden="true" class="w-5 h-5" />
                                        </span>
                            ...
                            <button class="p-2 ml-4 -m-2 text-gray-400 hover:text-gray-500 sm:ml-6 lg:hidden"
                                type="button" @click="mobileFiltersOpen = true">
                                <span class="sr-only">Filters</span>
                                <FunnelIcon aria-hidden="true" class="w-5 h-5" />
                            </button>
                        </div>
                    </div>
...
option, optionIdx
                                    ...
                                    <DisclosurePanel class="pt-6">
                                        <div class="space-y-4">
                                            <div v-for="option in attribute.options" class="flex items-center">
                                                ...
                                                <label :for="option.value"
                                                    class="ml-3 text-sm text-gray-600 first-letter:uppercase">
                                                    <template v-if="option.value == 1">{{ __("yes") }}
                                                    </template>
                                                    <template v-else>
                                                        {{ option.value }}
                                                    </template>
                            ...
                            <div v-if="products.data.length > 0" class="md:grid-cols-3 lg:col-span-5">
                                <div
                                    ...
                                    <div v-for="product in products.data">
                                        <div class="container-rounded bg-3 relative group/card xl:min-h-[27.5rem]">
                                            <div v-if="
                                            ...
                                            <div class="pb-2 hover:cursor-pointer">
                                                <div>
                                                    <div class="static">
                                                ...
                                                <div class="flex flex-col items-start">
                                                    ...
                                                    ">
                                                        <div class="flex flex-row space-x-1">
                                                            <p class="text-sm font-medium line-through font-mulish">
                                                                {{
                                                                    formatPrice(
                                                                        product.price
                                                                    )
                                                    ...
                                                    ">
                                                        <p class="text-xl font-medium font-mulish">
                                                            {{
                                                                formatPrice(
                                                                    product.promotion_price
                                                                )
                            ...
                            <div v-if="products.data.length <= 0" class="mx-auto lg:col-span-3">
                                <p class="py-12 text-2xl font-bold text-gray-500">
                                    {{ __("no_products") }}
                                </p>
                            </div>
                        </div>
