    ...
    <admin-layout :current-route="initialRoute" title="Products">
        <div class="grid w-full grid-cols-1 gap-4">
            <div class="relative container-rounded">
                <span class="absolute flex items-center justify-center gap-3 top-1 right-2">
                    <PencilSquareIcon v-if="!editProduct" @click="startEditProduct"
                        class="w-6 cursor-pointer text-slate-500 hover:text-slate-700" />
                    <TrashIcon @click="deleteProduct" class="w-6 cursor-pointer text-slate-500 hover:text-red-700" />
                </span>
                <div class="max-w-full px-4 mx-auto sm:px-6 lg:px-8">
                    <div class="flex flex-col -mx-4 md:flex-row">
                        <div class="px-4 md:flex-1">
                            <div class="h-[460px] rounded-lg border  dark:bg-gray-700 mb-4">
                                <img class="object-scale-down w-full h-full" :src="product.images_image1"
                                    alt="Product Image" />
                            </div>
                            <div class="flex mb-4 -mx-2">
                                <div class="flex gap-2 cursor-move" v-for="(image, index) in images" :key="index"
                                    draggable="true" @dragenter.prevent @dragstart="startDrag($event, index)"
                                    @drop="onDrop($event, index)" @dragover.prevent="onOver($event)"
                                    @dragleave.prevent="onLeave($event)">
                                    <div class="relative p-3 space-x-3 border rounded-md">
                                        <img :src="image" class="w-32 m-4 " :alt="`Image ${index + 1}`" />
                                        <TrashIcon @click="deleteImage(image)"
                                            class="absolute w-4 text-red-500 cursor-pointer bottom-6 left-3" />
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center gap-4 border" v-if="images.length != 4">
                                <input @change="
                                    uploadedImage = $event.target.files[0]
                                    "
                                    class="block text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                                    id="image" type="file" />
                                <SecondaryButton @click="submitImage">Încarcă</SecondaryButton>
                            </div>
                        </div>
                        <div class="px-4 md:flex-1">
                            ...
                            <div class="flex mb-4">
                                <div class="mr-4">
                                    <span class="font-bold text-gray-700 dark:text-gray-300">{{ __("sub_sub_category")
                                        }}:
                                    </span>
                                ...
                                <div class="mr-4">
                                    <span class="font-bold text-gray-700 dark:text-gray-300">{{ __("brand") }}:
                                    </span>
                                    <span class="text-gray-600 dark:text-gray-300">
                                        {{ product?.brand_name }}
                                    </span>
                            ...
                            <div class="flex mb-4">
                                <div class="mr-4">
                                    <span class="font-bold text-gray-700 dark:text-gray-300">{{ __("price") }}:
                                    </span>
                                    <span class="text-gray-600 dark:text-gray-300">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-2">
                                    <span v-for="credit in credits" v-if="credits.length" :key="credit.id"
                                        ...
                                        <TrashIcon @click="deleteCredit(credit.id)"
                            ...
                            <div class="mb-4">
                                <span class="font-bold text-gray-700 dark:text-gray-300">{{ __("installments")
                                    }}:</span>
                                <div class="flex items-center mt-2">
                                    <span v-for="installment in installments" v-if="installments.length"
                                        :key="installment.id"
                    ...
                    <Modal :show="isOpen" :closeable="true" @close="closeModal" :actions="false">
                        <div class="flex flex-col w-full gap-2 p-4 mt-8">
                            <div class="flex justify-between gap-2">
                                <black-input type="text" class="w-full" v-model="form.num_of_installments"
                                    label="Număr de luni" />
                                <black-input type="text" class="w-full" v-model="form.interest_rate"
                                    label="Rata dobânzii" />
                            </div>
                            <black-selector v-model="form.type" @update:status="form.type = $event" :options="[
                                {
                                    id: 'credit',
                                    value: 'credit',
                                    label: 'Credit',
                            ...
                            ]" :value="form.type" :error-message="__(form.errors.type)" :label="__('type')" />
                        </div>
                        <div class="flex justify-end mt-6 mb-4">
                            <SecondaryButton class="mx-2" @click="closeModal">
                                {{ __("cancel") }}
                            </SecondaryButton>
                            <PrimaryButton @click="submitCredits" class="mx-2">{{ __("submit") }}</PrimaryButton>
                        </div>
                    </Modal>
                    <Modal :show="editProduct" :closeable="true" @close="closeEditProduct" :actions="false">
                        <div class="flex flex-col w-full gap-2 p-4 mt-8 h-3/4">
                            <div class="flex justify-between gap-2">
                                <black-input type="text" class="w-full" v-model="productForm[
                                    `name ${page.props.current_locale}`
                                ]
                                    " :label="__('title')" />
                            </div>
                            <black-input type="textarea" class="w-full h-auto" v-model="productForm[
                                `description ${page.props.current_locale}`
                            ]
                                " :label="__('description')" />
                            <black-selector v-model="productForm.category_id" @update:status="
                                productForm.category_id = $event
                                " :options="subSubCategories" :value="productForm.category_id" :error-message="__(form.errors.category_id)
                                    " :label="__('sub_sub_category')" />
                            <black-selector v-model="productForm.brand_id"
                                @update:status="productForm.brand_id = $event" :options="brands"
                                :value="productForm.brand_id" :error-message="__(form.errors.brand_id)"
                                :label="__('brand')" />
                            <black-input type="number" class="w-full h-auto" v-model="productForm.price"
                                :label="__('price')" />
                        </div>
                        <div class="flex justify-end mt-6 mb-4">
                            <SecondaryButton class="mx-2" @click="closeEditProduct">
                                {{ __("cancel") }}
                            </SecondaryButton>
                            <PrimaryButton @click="submitEditProduct" class="mx-2">{{ __("submit") }}</PrimaryButton>
                        </div>
                    </Modal>
                </div>
            </div>
