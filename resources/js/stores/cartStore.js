...
export const useCartStore = defineStore("cart", () => {
    ...
    async function addProductInCart(productId, colorId) {
        ...
        axios
            .get(
                route("api.cartAdd", { productId: productId, colorId: colorId })
            )
            .then(async (response) => {
    ...
    async function updateQtyOfProduct(productId, qty) {
        axios
            .get(
                route("api.cart.updateQtyOfProduct", {
                    productId: productId,
                    qty: qty,
                })
            )
    ...
    async function removeProductInCart(productId) {
        axios
            .delete(route("api.cartRemove", { productID: productId }))
            .then(async (response) => (message.value = response.data))
            .finally(() => fetchCount());
    }
    ...
    async function fetchCount() {
        try {
            ...
            console.error(
                "A apărut o eroare în timpul solicitării pentru numărul de produse din coș:",
                error
            );
        }
    }
