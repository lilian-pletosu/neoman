...
class NomenclatureImportJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        do {
            ...
            Log::info("Status for NOMENCLATURE is: ", ['status' => $status['status'] ?? null]);
            if (isset($status['status']) && !$status['status']) {
                Log::info('Service not yet ready', ['status' => $status['status'] ?? null]);
        ...
        $this->isCommit();
        Log::info("Data for NOMENCLATURE received", ['data' => $data]);

        // Salvăm datele în Redis
        Redis::set("NOMENCLATURE", json_encode($data));
